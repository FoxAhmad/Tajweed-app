...
def save_audio():
    try:
        ...
        if not letter:
            return jsonify({"error": "Letter name required"}), 400

        # Create temp directory if needed
        os.makedirs(INPUT_DIR, exist_ok=True)
        
        # Save original file temporarily
        temp_path = os.path.join(INPUT_DIR, f"temp_{datetime.now().timestamp()}")
        final_path = os.path.join(INPUT_DIR, "input.wav")
        audio_file.save(temp_path)
        
        # Convert to proper WAV format
        if not convert_to_wav(temp_path, final_path):
        ...
        return jsonify({
            "success": True,
            "message": "Audio saved and converted successfully",
            "path": final_path,
            "letter": letter
        })
